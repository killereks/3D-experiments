...
class Quaternion:
    ...
    def ToEuler(self):
        heading = np.arctan2(2 * self.y * self.w - 2 * self.x * self.z, 1 - 2 * self.y ** 2 - 2 * self.z ** 2)
        attitude = np.arcsin(2 * self.x * self.y + 2 * self.z * self.w)
        bank = np.arctan2(2 * self.x * self.w - 2 * self.y * self.z, 1 - 2 * self.x ** 2 - 2 * self.z ** 2)

        heading = np.degrees(heading)
        attitude = np.degrees(attitude)
        bank = np.degrees(bank)
    ...
    def MultiplyVector(q, v):
        ...
        return np.array([w * q.x + x * q.w - y * q.z + z * q.y, w * q.y + y * q.w - z * q.x + x * q.z, w * q.z + z * q.w - x * q.y + y * q.x])
